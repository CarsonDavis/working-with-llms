...
This is the foundation, and it's the easy part. Get it right once and it mostly takes care of itself, then spend your real energy on context engineering and process.

## The model

For most tasks that aren't tightly bounded, use a powerful flagship model, and when in doubt reach for the most capable one you have. We have the most experience with Claude Opus 4.8, but the latest flagships from OpenAI and Gemini drop in just as well. Don't chase leaderboards: the "best" model changes every few weeks, and the gap between the top flagships is almost never what makes or breaks a task. For small, mechanical, tightly-scoped work a cheaper or faster model is fine; for anything that needs real judgment, use the strongest one available and give it access to as much context as you can.
...
Here's the loop superpowers runs when you hand it a task:
...
Many on our team run a stack of layers: Claude Code (the agentic harness itself) on top of Opus 4.8, and then superpowers-extended-cc on top of that. It's a Claude Code-specific fork of [obra/superpowers](https://github.com/obra/superpowers), installed from [pcvelz/superpowers](https://github.com/pcvelz/superpowers).
