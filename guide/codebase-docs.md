...
- **Big context does not equal good decisions.** Five thousand tokens of code might reduce to a one-paragraph fact, and the agent acts on the paragraph far more reliably than on the code.
...
- **Name real symbols.** Reference actual files and functions, not prose-only descriptions, so an implementing agent can act on what you wrote.
