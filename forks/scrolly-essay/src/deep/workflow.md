...
The core loop is small: the agent writes a spec from your issue and the current code, an implementer builds to that spec, and a fresh agent reviews the result against it. [The essay](/#workflow) walks the full sequence; the point here is that there is more than one way to make that loop happen.
