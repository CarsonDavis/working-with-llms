---
title: "Harness & Model"
dek: "For most tasks that aren't tightly bounded, use a powerful flagship model, and when in doubt reach for the most capable one you have."
part: "Foundation"
order: 2
reviewed: 2026-07-02
---
...
For most tasks that aren't tightly bounded, use a powerful :term[flagship model], and when in doubt reach for the most capable one you have. We have the most experience with Claude Opus 4.8, but the latest flagships from OpenAI and Gemini drop in just as well.
...
A raw model is a fancy next-word predictor. Give it a feature to build and it might start strong, drift halfway through, and then hand you something almost-but-not-quite right. A :term[harness] fixes that by turning "build me this" into a gated pipeline: each step is pinned to something concrete, and nothing moves forward until it checks out.
...
:::steps
1. **Spec first.** It explores your actual code, asks you questions, proposes an approach, and writes a design :term[spec]. Nothing gets built until you approve it.
...
You can run all of this in an isolated git :term[worktree], so several tasks can go at once without stepping on each other.
...
:::recommendation
Run your agent on your actual machine, not in a browser tab.
:::

You want it in your real environment, with your tools and your repo, able to run and smoke-test what it builds.
